...
import com.aquaspoof.unified.toolkit.mcpe.databinding.ActivityConfigDetailBinding
...
import com.aquaspoof.unified.toolkit.mcpe.R

class ConfigDetailActivity : AppCompatActivity() {

    private lateinit var binding: ActivityConfigDetailBinding
    private var configItem: ConfigItem? = null
    private var downloadJob: Job? = null

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        binding = ActivityConfigDetailBinding.inflate(layoutInflater)
        setContentView(binding.root)

        val jsonString = intent.getStringExtra("config_json_data")
        if (jsonString != null) {
            try {
                configItem = Gson().fromJson(jsonString, ConfigItem::class.java)
            } catch (_: Exception) {}
        }

        if (configItem == null) {
            Toast.makeText(this, "Ошибка: данные конфига не получены", Toast.LENGTH_SHORT).show()
            finish()
            return
        }
        ...
        binding.buttonCompare.setOnClickListener { compareHashes() }
    }

    private fun copyToClipboard(label: String, text: String?) {
        if (text.isNullOrEmpty() || text == getString(R.string.detail_not_specified)) return
    ...
    private fun startDownload() {
        ...
        downloadJob = lifecycleScope.launch {
            ConfigDownloader.download(this@ConfigDetailActivity, item).collect { state ->
                when (state) {
                    is DownloadState.Progress -> {
                        val downloadedStr = Formatter.formatShortFileSize(this@ConfigDetailActivity, state.downloadedBytes)
                        val totalStr = Formatter.formatShortFileSize(this@ConfigDetailActivity, state.totalBytes)
                        val speedStr = Formatter.formatShortFileSize(this@ConfigDetailActivity, state.speedBps)
                    ...
                    is DownloadState.Success -> {
                        resetDownloadUI()
                        checkFileStatus()
                        Toast.makeText(this@ConfigDetailActivity, "Загружено: ${state.filename}", Toast.LENGTH_SHORT).show()
                    }
                    is DownloadState.Error -> {
                        resetDownloadUI()
                        checkFileStatus()
                        Toast.makeText(this@ConfigDetailActivity, "Ошибка: ${state.message}", Toast.LENGTH_LONG).show()
                    }
    ...
    private fun checkFileStatus() {
        ...
        lifecycleScope.launch(Dispatchers.IO) {
            ...
            val localVersion = prefs.getString("config_version_${item.id}", null)
            ...
            if (folderUriString != null) {
                try {
                    ...
                    val folder = DocumentFile.fromTreeUri(this@ConfigDetailActivity, folderUri)
                    val existingFile = folder?.findFile(item.filename)
    ...
    private fun calculateAndDisplayLocalHashes(uri: Uri) {
        lifecycleScope.launch {
            try {
                val hashes = HashCalculator.calculateHashes(this@ConfigDetailActivity, uri)
                ...
                binding.tvLocalMd5.setOnClickListener { copyToClipboard("MD5", hashes.md5) }
                binding.tvLocalSha1.setOnClickListener { copyToClipboard("SHA1", hashes.sha1) }
                binding.tvLocalSha256.setOnClickListener { copyToClipboard("SHA256", hashes.sha256) }
                binding.tvLocalCrc32.setOnClickListener { copyToClipboard("CRC32", hashes.crc32) }
    ...
    private fun compareHashText(serverView: TextView, localView: TextView, serverHash: String?): Boolean {
        ...
        if (serverHash == null || localHash == null) {
            serverView.setTextColor(Color.GRAY); localView.setTextColor(Color.GRAY)
            return false
        }
        if (serverHash.equals(localHash, ignoreCase = true)) {
            serverView.setTextColor(Color.GREEN); localView.setTextColor(Color.GREEN)
            return true
        } else {
            serverView.setTextColor(Color.RED); localView.setTextColor(Color.RED)
            return false
        }
    }
    ...
    private fun updateButtonUI(fileExists: Boolean, localVersion: String?, serverVersion: String) {
        ...
        when {
            !fileExists -> binding.buttonDownload.text = getString(R.string.script_button_download)
            localVersion?.trim() != serverVersion.trim() -> binding.buttonDownload.text = getString(R.string.script_button_update)
            else -> {
                binding.buttonDownload.text = getString(R.string.script_button_downloaded)
                binding.buttonDownload.isEnabled = false
            }
    ...
    private fun setupToolbar() {
        ...
        supportActionBar?.title = configItem?.name ?: "Config"
    }
    ...
    private fun populateUi() {
        ...
        val varVer = item.variableVersion ?: na
        binding.tvVariableVersion.text = "Variable: $varVer"
        ...
        binding.tvFilesize.text = getString(R.string.detail_label_filesize, Formatter.formatShortFileSize(this, item.fileSize))
